...
export default function ToggleSwitch({
  ...
  return (
    <label className="relative inline-flex cursor-pointer items-center">
      ...
      <div
        className={`relative h-8 w-18 rounded-full transition-all`}
        style={{ backgroundColor: isOn ? trueBg : falseBg }}
      >
        <span
          className={`absolute top-1 left-1 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow transition-transform ${isOn ? "translate-x-10" : "translate-x-0"}`}
        >
